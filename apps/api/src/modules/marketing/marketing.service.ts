...
export class MarketingService {
  ...
  async createCampaign(data: {
    name: string; objective?: string; channel: MarketingChannel;
  ...
  async updateCampaign(id: string, data: Partial<{
    name: string; objective: string; budget: number;
    status: CampaignStatus; spent: number;
  }>) {
    return this.prisma.campaign.update({ where: { id }, data, include: { kpi: true } });
  }

  async updateKPI(campaignId: string, data: {
    impressions?: number; clicks?: number; cpc?: number;
    ctr?: number; cpa?: number; revenue?: number; roas?: number;
  }) {
  ...
  async createPromoCode(data: {
    code: string; discountType: PromoDiscountType; discountValue: number;
    minOrderValue?: number; maxDiscount?: number;
    usageLimit?: number; expiryDate?: string; campaignId?: string;
  }) {
    const exists = await this.prisma.promoCode.findUnique({ where: { code: data.code.toUpperCase() } });
  ...
  async validatePromoCode(code: string, orderTotal: number) {
    ...
    if (promo.expiryDate && promo.expiryDate < new Date()) throw new BadRequestException('Promo code expired');
    if (promo.usageLimit && promo.usageCount >= promo.usageLimit) throw new BadRequestException('Promo code usage limit reached');
    if (promo.minOrderValue && orderTotal < promo.minOrderValue) throw new BadRequestException(`Minimum order value is ${promo.minOrderValue}`);
    ...
    return { valid: true, promoCodeId: promo.id, discount, discountType: promo.discountType, discountValue: promo.discountValue };
  }
  ...
  async findAllInfluencers(page = 1, limit = 20) {
    ...
    return { data: influencers, meta: { total, page, limit, totalPages: Math.ceil(total / limit) } };
  }

  async createInfluencer(data: {
    name: string; handle: string; platform: string;
    followers?: number; email?: string; phone?: string;
    campaignId?: string; promoCode?: string; commission?: number;
  }) {
    return this.prisma.influencer.create({ data });
  }

  async updateInfluencer(id: string, data: Partial<{
    name: string; followers: number; revenue: number;
    campaignId: string; promoCode: string; commission: number;
  }>) {
